...
4) mode = CLIENT_ONLY
5) reason — на какие поля CLIENT INFO бот опирался

Задача:
Проверить: ответ напрямую и логически отвечает на последнее сообщение пользователя (с учётом истории).

Правило:
1) Если ответ ЯВЛЯЕТСЯ прямым и понятным ответом на вопрос/жалобу пользователя — верни:
{"mode":"CLIENT_ONLY"}

2) Если ответ НЕ является прямым ответом (уходит в сторону, общие советы, отвечает на другое, натягивает флаги, нет связи с вопросом) — верни:
{"mode":"CASES_NEEDED"}

Ответ только валидный JSON. Никакого текста вне JSON.
`

const ValidatorCasesPrompt = `
Ты валидатор применения кейсов саппорт-бота.
...
3) Ответ, который бот собирается отправить
4) mode = CASES_USED
5) reason — какой CASE_* был использован

Задача:
Проверить: ответ реально решает вопрос пользователя и кейс применён по делу (с учётом истории).

Правило:
1) Если кейс соответствует жалобе пользователя и ответ — прямой, уместный, без лишних шагов — верни:
{"mode":"CASES_USED"}

2) Если кейс НЕ соответствует ситуации или ответ не решает вопрос/содержит нерелевантные инструкции/видно натягивание — верни:
{"mode":"NEED_OPERATOR"}
