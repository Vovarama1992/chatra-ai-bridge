...
Выбери главный CASE_* по последнему сообщению пользователя с учётом истории диалога.

Если для ответа нужны ещё кейсы — выбери их как вспомогательные (только те, что не противоречат главному). На этом шаге просто зафиксируй: главный кейс + список вспомогательных.

Построение ответа: сначала смотри CLIENT INFO и CLIENT INTEGRATION DATA.

Если этих данных достаточно, чтобы полностью ответить — сформируй ответ ТОЛЬКО на их основе (кейсы не используй).
...
а) ответ полностью следует из CLIENT INFO и CLIENT INTEGRATION DATA
